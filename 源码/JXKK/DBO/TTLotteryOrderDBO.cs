...
namespace Com.ZY.JXKK.DBO
{
    ...
    /// Author:代码生成器
    /// </summary>
    public class TTLotteryOrderDBO
    {
        ...
        public virtual void Add(DataAccess data,TTLotteryOrder ttLotteryOrder)
        {
            string strSQL = "insert into TTLotteryOrder (orderId,agentId,siteId,terminalId,clientId,clientName,cardId,gameId,lotteryType,period,userBetCodes,centerBetCodes,multiple,betFee,betTime,isChase,chaseOrderId,orderStatus,orderTime,awardResult,awardMoney,awardTime,settleStatus,settleTime,createTime,srcType) values (@orderId,@agentId,@siteId,@terminalId,@clientId,@clientName,@cardId,@gameId,@lotteryType,@period,@userBetCodes,@centerBetCodes,@multiple,@betFee,@betTime,@isChase,@chaseOrderId,@orderStatus,@orderTime,@awardResult,@awardMoney,@awardTime,@settleStatus,@settleTime,To_date(@createTime,'yyyy-mm-dd hh24:mi:ss'),@srcType)";
        ...
        /// </summary>
        /// <param name="ttLotteryOrder">彩票订单</param>
        public virtual void Edit(TTLotteryOrder ttLotteryOrder)
        {
            try
            {
                db.Open();
        ...
        public virtual List<TTLotteryOrder> GetList(string strSQL,Param param)
        {
        ...
        /// <returns>彩票订单列表对象</returns>
        public virtual List<TTLotteryOrder> GetList(string field, string value)
        {
            List<TTLotteryOrder> list = new List<TTLotteryOrder>();
            try
            {
        ...
        /// <returns>彩票订单对象</returns>
        protected virtual TTLotteryOrder ReadData(ComDataReader dr)
        {
            ...
            ttLotteryOrder.userBetCodes= dr["userBetCodes"].ToString();//用户投注串
            ttLotteryOrder.centerBetCodes= dr["centerBetCodes"].ToString();//中心投注串
